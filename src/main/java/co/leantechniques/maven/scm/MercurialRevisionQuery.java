...
public class MercurialRevisionQuery implements ScmRevisionQuery {
    @Override
    public boolean supports(File directory) {
        return new File(directory, ".hg").exists();
    }

    @Override
    public CodeRevision queryRevision(File directory) {
        return new CodeRevision(
                "hg", new Cmd("hg").arg("id").arg("-i").execute()
        );
    }
}
